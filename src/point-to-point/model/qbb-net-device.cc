...
uint32_t GetQpTracePsn(Ptr<RdmaQueuePair> qp, uint32_t mtu) {
    uint32_t packetSize = qp->psnPath.packetSize == 0 ? mtu : qp->psnPath.packetSize;
    if (packetSize == 0) return 0;
    if (qp->ornic.enabled && !qp->ornic.pendingRetrans.empty()) {
        return qp->ornic.pendingRetrans.front() / packetSize;
    }
    if (qp->psnPath.pathAwareRetransEnabled &&
        !qp->psnPath.pendingRetrans.empty()) {
        return qp->psnPath.pendingRetrans.front().startPsn;
    }
    return static_cast<uint32_t>(qp->snd_nxt / packetSize);
}

bool ShouldTraceQpScheduling(Ptr<RdmaQueuePair> qp, uint32_t mtu) {
    if (Settings::enable_console_log != 1 || Settings::console_log_psns.empty()) {
        return false;
    }
    uint32_t psn = GetQpTracePsn(qp, mtu);
    std::string normalized = Settings::console_log_psns;
...
std::string FormatRetransFront(Ptr<RdmaQueuePair> qp) {
    if (qp->ornic.enabled && !qp->ornic.pendingRetrans.empty()) {
        uint32_t packetSize = qp->ornic.packetSize == 0 ? 1 : qp->ornic.packetSize;
        std::ostringstream oss;
        oss << "[ORNIC:" << (qp->ornic.pendingRetrans.front() / packetSize) << "]";
        return oss.str();
    }
    if (!qp->psnPath.pathAwareRetransEnabled ||
        qp->psnPath.pendingRetrans.empty()) {
...
int RdmaEgressQueue::GetNextQindex(bool paused[]) {
    ...
    for (qIndex = 1; qIndex <= fcount; qIndex++) {
        ...
        bool hasOrnicRetrans =
            qp->ornic.enabled &&
            !qp->ornic.pendingRetrans.empty();
        bool hasPendingRetrans =
            qp->psnPath.pathAwareRetransEnabled &&
            !qp->psnPath.pendingRetrans.empty();
        bool hasGoBackRetrans = qp->snd_nxt < qp->stat.txHighestSeqSent;
        bool hasRetransToSend = hasOrnicRetrans || hasPendingRetrans || hasGoBackRetrans;
        bool cond_window_allowed =
            hasRetransToSend ||
            (!qp->IsWinBound() && (!qp->irn.m_enabled || qp->CanIrnTransmit(m_mtu)));
        bool cond2 = ((qp->GetBytesLeft() > 0 || hasRetransToSend) && cond_window_allowed);

        if (ShouldTraceQpScheduling(qp, m_mtu)) {
            ...
            uint64_t bytesLeftPkts = packetSize == 0 ? 0 : qp->GetBytesLeft() / packetSize;
            uint64_t winPkts = packetSize == 0 ? 0 : win / packetSize;
            uint64_t onTheFlyPkts = packetSize == 0 ? 0 : onTheFlyBytes / packetSize;
            std::cout << "[PSN_TRACE_SCHED_BLOCK] t=" << Simulator::Now().GetNanoSeconds()
                      << " qp=" << qp->m_flow_id
                      << " nextPsn=" << GetQpTracePsn(qp, m_mtu)
                      << " snd_nxt_psn=" << sndNxtPsn
                      << " snd_una_psn=" << sndUnaPsn
                      << " snd_nxt_bytes=" << qp->snd_nxt
                      << " snd_una_bytes=" << qp->snd_una
                      << " paused=" << (!cond1 ? 1 : 0)
                      << " hasPendingRetrans=" << (hasPendingRetrans ? 1 : 0)
                      << " hasGoBackRetrans=" << (hasGoBackRetrans ? 1 : 0)
                      << " hasRetransToSend=" << (hasRetransToSend ? 1 : 0)
                      << " retransFront=" << FormatRetransFront(qp)
                      << " bytesLeftPkts=" << bytesLeftPkts
                      << " bytesLeft=" << qp->GetBytesLeft()
                      << " isWinBound=" << (qp->IsWinBound() ? 1 : 0)
                      << " irnEnabled=" << (qp->irn.m_enabled ? 1 : 0)
                      << " irnCanTransmit=" << ((!qp->irn.m_enabled || qp->CanIrnTransmit(m_mtu)) ? 1 : 0)
                      << " condWindowAllowed=" << (cond_window_allowed ? 1 : 0)
                      << " condReady=" << (cond2 ? 1 : 0)
                      << " nextAvailNs=" << qp->m_nextAvail.GetNanoSeconds()
